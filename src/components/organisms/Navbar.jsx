...
import { useCheckoutModal } from "@/store/CheckoutModal";
    ...
    <header className="sticky top-0 z-40 w-full nav-blur relative">
      <div className="w-full max-w-screen-2xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 md:gap-4 min-w-0">
          ...
          <Logo nameClassName="hidden sm:flex" />
          <nav className="hidden lg:flex items-center ml-3 xl:ml-5">
                              ...
                              <>
                                ...
                                {searchResults.products.map((p) => (
                                  <Link
                                    key={p._id}
                                    to={`/products/${p._id}`}
                                    state={{ backgroundLocation: location, product: p }}
                                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-muted transition-colors"
                                  >
                                    <img
...
const NavPill = ({ to, label, isActive: active, accent }) => (
  <Link
    to={to}
    className={`group relative px-2.5 xl:px-3.5 py-2 text-sm font-medium transition-colors duration-200 ${
      active
        ? accent
          ? "text-[#9E2B5E]"
          : "text-foreground"
  ...
  >
    ...
    <span
      className={`absolute left-2.5 right-2.5 xl:left-3.5 xl:right-3.5 -bottom-0.5 h-[2px] rounded-full bg-gradient-to-r from-[#D2691E] to-[#9E2B5E] origin-left transition-transform duration-200 ${
...
const CategoryDropdown = ({ categories }) => {
  ...
  return (
    ...
    >
      <button
        className={`group relative px-2.5 xl:px-3.5 py-2 text-sm font-medium transition-colors duration-200 flex items-center gap-1 ${
...
const CartSheet = ({ cart, cartCount }) => {
    ...
    {open && (
      <>
        ...
        <motion.div
          ...
          className="fixed top-0 left-0 z-50 h-full w-[min(20rem,85vw)] bg-card shadow-elevated lg:hidden flex flex-col"
        >
