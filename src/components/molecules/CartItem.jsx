...
const CartItem = ({ item }) => {
  ...
  return (
    <div className="flex gap-4 py-4 border-b border-stone-100 last:border-0 animate-fade-up group">
      <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-stone-50 shrink-0 shadow-sm">
        <img
        ...
        <div>
          <div className="flex items-start justify-between gap-2">
            ...
            <button
              ...
              className="p-1 rounded-lg text-stone-300 hover:text-red-500 hover:bg-red-50 transition-all opacity-0 group-hover:opacity-100"
            >
              <Trash2 className="h-3.5 w-3.5" />
            </button>
          </div>
          {item.variant && (
            <p className="text-[11px] text-stone-400 mt-0.5">{item.variant}</p>
          )}
        </div>
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-1 border border-stone-200 rounded-lg overflow-hidden">
            <button
              ...
              disabled={localQty <= 1}
              className="w-7 h-7 flex items-center justify-center text-stone-500 hover:bg-stone-50 transition-colors disabled:opacity-30"
            >
              <Minus className="h-3 w-3" />
            </button>
            <span className="w-7 text-center text-sm font-bold text-stone-800">{localQty}</span>
            <button
              onClick={() => handleUpdate(1)}
