...
import { useMyOrders, useCancelOrder, useUpdateAddress, useDeliveryOtp } from "@/hooks/useOrders";
import { useToast } from "../store/Toast";
import { formatPrice, formatDate, getStatusColor } from "@/lib/utils";
import { MapPin, Pencil, Copy, Check, Shield, Phone, User, Mail, Package, X, Clock } from "lucide-react";
...
const AddressEditDialog = ({ order }) => {
  ...
  const handleSubmit = (e) => {
    ...
    updateAddress.mutate(
      ...
      {
        onSuccess: () => { toast({ title: "Address updated successfully" }); setOpen(false); },
        onError: (err) => toast({ title: err.response?.data?.message || "Failed to update", variant: "destructive" }),
      },
    );
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <FormField label="Address Line 1" value={form.line1} onChange={set("line1")} placeholder="123 Main St" required />
          <FormField label="Address Line 2" value={form.line2} onChange={set("line2")} placeholder="Apartment, suite" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Postal Code" value={form.postalCode} onChange={set("postalCode")} placeholder="400001" required />
            <FormField label="Country" value={form.country} onChange={set("country")} placeholder="IN" required />
          </div>
  ...
  return (
    <div className="mt-3 p-4 rounded-2xl bg-indigo-50 border-2 border-indigo-200">
      <div className="flex items-center gap-2 mb-2">
        ...
        <p className="text-xs font-semibold text-indigo-800 uppercase tracking-wider">Delivery OTP</p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="icon" onClick={handleCopy} className="h-12 w-12 border-indigo-300 hover:bg-indigo-100 shrink-0 rounded-xl">
          {copied ? <Check className="h-5 w-5 text-green-600" /> : <Copy className="h-5 w-5 text-indigo-600" />}
        </Button>
      </div>
    </div>
  );
    ...
    <Card className="animate-fade-up overflow-hidden">
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full capitalize ${getStatusColor(order.orderStatus)}`}>
        ...
        {order.customerName && (
          <div className="mt-3 flex items-center gap-3 text-xs text-stone-500">
            ...
            {order.customerPhone && <><span className="text-stone-200">|</span><span className="flex items-center gap-1"><Phone className="h-3 w-3" />{order.customerPhone}</span></>}
          </div>
        )}
        ...
        <div className="mt-3 space-y-1">
          {order.items?.map((item) => (
            <p key={item._id} className="text-sm text-stone-600">
              <span className="font-medium text-stone-900">{item.product?.name}</span> × {item.quantity}
            </p>
          ))}
        </div>
          ...
          <div className="mt-3 p-3 rounded-2xl bg-stone-50 border border-stone-100 space-y-1">
            ...
            {order.deliveryDistance > 0 && (
              <p className="text-[10px] text-stone-400 pl-6">
                ...
                {order.estimatedDeliveryTime && <span className="ml-2">~{order.estimatedDeliveryTime}</span>}
              </p>
            )}
            {order.orderNotes && (
        ...
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between text-stone-500">
            <span>Delivery{order.deliveryDistance > 0 && <span className="text-stone-400 ml-1">({order.deliveryDistance} km)</span>}</span>
            <span className="text-stone-900">{order.deliveryFee > 0 ? formatPrice(order.deliveryFee) : <span className="text-emerald-600 font-medium">FREE</span>}</span>
          </div>
        </div>
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-stone-100">
          <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-md ${order.paymentMethod === "cod" ? "bg-amber-50 text-amber-700" : "bg-sky-50 text-sky-700"}`}>
