...
const FOOD_TYPES = ["veg", "nonveg", "vegan", "jain"];
    ...
    <div className="h-32 sm:h-36 overflow-hidden bg-muted relative">
      ...
      {!product.isAvailable && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <span className="text-white font-semibold text-xs sm:text-sm bg-destructive px-2 py-1 rounded">
            Unavailable
          </span>
        </div>
      )}
...
const OwnerProducts = () => {
  ...
  const handleCreate = (e) => {
    ...
    fd.append("isVeg", String(form.foodType !== "nonveg"));
  ...
  const handleQuickPrice = (id, data) =>
  ...
  const handleToggle = (id, isAvailable) => {
    setAvailability.mutate(
      { id, isAvailable },
      {
        onSuccess: () => {
          refetchOwnerProducts();
          toast({ title: isAvailable ? "Product is now in stock" : "Product marked out of stock" });
        },
      },
    );
  };
  ...
  const handlers = {
    ...
    loading: updateProduct.isLoading || deleteProduct.isLoading,
  };
  ...
  <>
    ...
    <div className="space-y-1">
      ...
      {categories?.length > 0 ? (
        <Select
          value={form.category}
          onValueChange={(v) => onChange("category")({ target: { value: v } })}
        >
          <SelectTrigger>
            <SelectValue placeholder="Select a category" />
          </SelectTrigger>
    ...
    <div className="space-y-1">
      ...
      <Select
        value={form.foodType}
        onValueChange={(v) => onChange("foodType")({ target: { value: v } })}
      >
        <SelectTrigger>
