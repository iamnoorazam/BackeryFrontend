...
const ProductDetail = () => {
  ...
  const handleBuyNow = () => {
    ...
    // Express checkout: snapshot this product + chosen variant/qty and open the
    // Buy Now checkout (order summary + delivery details) — never the cart.
    ...
    >
      ...
      <nav aria-label="Breadcrumb" className="mb-4 md:mb-6">
        <ol className="flex items-center gap-1.5 text-xs sm:text-sm text-muted-foreground flex-wrap">
          ...
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/50" />
          <li className="font-semibold text-foreground truncate max-w-[160px] sm:max-w-xs">
            {product.name}
          </li>
        </ol>
      </nav>
      ...
      <motion.div variants={itemVariants} className="grid md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-3">
          <div
            className={`relative overflow-hidden rounded-2xl bg-muted border border-border ${isFashion ? "aspect-[3/4]" : "aspect-square"}`}
          >
            ...
            {/* Zoom hint */}
            <div className="absolute top-4 right-4 w-8 h-8 rounded-full bg-card/80 backdrop-blur-sm flex items-center justify-center shadow-md pointer-events-none">
              <ZoomIn className="h-4 w-4 text-foreground" />
            </div>
            {discount > 0 && (
              <div className="absolute top-4 left-4">
                <div className="bg-danger text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg flex items-center gap-1.5">
                  🔥 {discount}% OFF
                </div>
              </div>
            )}
            {images.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-card/80 backdrop-blur-sm hover:bg-card flex items-center justify-center shadow-lg transition-all"
                >
                  <ChevronLeft className="h-4 w-4 text-foreground" />
                </button>
                <button
                  onClick={() => setSelectedImage((i) => (i + 1) % images.length)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-card/80 backdrop-blur-sm hover:bg-card flex items-center justify-center shadow-lg transition-all"
                >
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {images.map((img, i) => (
                <button
                  ...
                  className={`shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${i === selectedImage ? "border-primary ring-1 ring-primary" : "border-border hover:border-foreground/30"}`}
                >
                  {" "}
                  {}
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              {product.isVeg !== undefined && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase ${product.isVeg ? "bg-success-subtle text-success border border-success/30" : "bg-danger-subtle text-danger border border-danger/30"}`}
                >
                  <span
                    className={`w-2 h-2 rounded-full ${product.isVeg ? "bg-success" : "bg-danger"}`}
                  />
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground/70">({product.totalReviews || 0} reviews)</span>
            </div>
          </div>
          ...
          <div className="flex items-baseline gap-3">
            ...
            {discount > 0 && (
              <span className="text-sm font-bold text-danger bg-danger-subtle px-2 py-0.5 rounded-lg">
                {discount}% off
              </span>
            )}
          </div>
          ...
          {product.allergens?.length > 0 && (
            <div className="bg-warning-subtle border border-warning/30 rounded-xl p-3">
              <p className="text-xs font-semibold text-warning">⚠ Allergen Info</p>
              ...
              <div className="flex flex-wrap gap-2">
                {product.variants.map((v) => (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${selectedVariant?.name === v.name ? "border-primary bg-primary/10 text-primary shadow-sm" : "border-border text-muted-foreground hover:border-foreground/30"}`}
                  >
                    <span>{v.name}</span>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-center justify-center border border-border rounded-xl overflow-hidden shrink-0 self-start sm:self-auto">
              <button
                onClick={() => setQty(Math.max(1, qty - 1))}
                className="h-12 w-12 flex items-center justify-center hover:bg-muted transition-colors text-muted-foreground"
              >
            ...
            <motion.div whileTap={{ scale: 0.95 }} className="w-full sm:w-auto">
              <Button
                ...
                className="w-full sm:w-auto bg-foreground hover:opacity-90 text-background border-0 shadow-lg text-sm font-bold py-6 px-6 rounded-xl gap-2 active:scale-95 transition-transform"
              >
      ...
      <motion.div variants={itemVariants} className="mt-12">
        ...
        {user?.role === "customer" && (
          <form
            ...
            className="border border-border rounded-xl p-5 space-y-4 bg-card mb-6"
          >
            ...
            <textarea
              ...
              required
              rows={3}
              className="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#D2691E] resize-none"
            />
            <Button
              type="submit"
        ...
        <div className="space-y-3">
          {reviews?.length > 0 ? (
            ...
            <p className="text-muted-foreground/70 text-sm py-8 text-center">
      ...
      <motion.div
        initial={{ y: 100 }}
        animate={{ y: 0 }}
        className="fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-xl border-t border-border p-4 md:hidden shadow-2xl"
      >
        <div className="flex items-center gap-2 max-w-lg mx-auto">
          <div className="flex items-center border border-border rounded-xl overflow-hidden shrink-0">
            ...
            <span className="px-3 text-sm font-bold text-foreground min-w-[24px] text-center">
              {qty}
            </span>
            <button onClick={() => setQty(qty + 1)} className="p-2.5 hover:bg-muted">
          ...
          <Button
            onClick={handleBuyNow}
            disabled={addToCart.isLoading || clearCart.isLoading}
            className="bg-foreground hover:opacity-90 text-background border-0 shadow-lg py-6 px-4 rounded-xl text-sm font-bold min-h-[52px] gap-1.5"
          >
      ...
      <Dialog open={zoomOpen} onOpenChange={setZoomOpen}>
        <DialogContent className="max-w-3xl p-4 sm:p-3 bg-card">
            ...
            <div className="flex justify-center gap-2 pt-1">
              {images.map((img, i) => (
                <button
                  key={i}
                  onClick={() => setSelectedImage(i)}
                  className={`w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${i === selectedImage ? "border-[#D2691E]" : "border-border hover:border-foreground/30"}`}
                >
                  <img src={img} alt="" className="w-full h-full object-cover" />
                </button>
              ))}
