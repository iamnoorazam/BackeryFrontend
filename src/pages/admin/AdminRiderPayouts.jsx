...
import { useAdminRiderPayouts, useProcessRiderPayout } from "@/hooks/useRiderEarnings";
...
const AdminRiderPayouts = () => {
  ...
  const { data: payouts, isLoading } = useAdminRiderPayouts(status === "all" ? undefined : status);
  const process = useProcessRiderPayout();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Rider Payouts ({payouts?.length ?? 0})</h1>
      ...
      {!payouts?.length ? (
        <EmptyState icon="💸" title="No rider payout requests" />
      ) : (
              ...
              <CardContent className="p-4 flex items-center gap-3 flex-wrap">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase text-stone-400">{p.method}</span>
                  </div>
                  <p className="text-xs text-stone-500 truncate">
                    {p.user?.name} ({p.user?.email}) · {formatDate(p.createdAt)}
                  </p>
                </div>
